...
const SENDER = { name: "ScoutMe", email: "noreply@scoutme-notification.com" };
...
export function generateOtp(email: string, secret: string, windowIndex?: number): string {
  const window = windowIndex ?? Math.floor(Date.now() / (WINDOW_MINUTES * 60 * 1000));
  const raw = createHmac("sha256", secret).update(`${email}:${window}`).digest("hex");
...
function buildEmail(otp: string, role: string, firstName: string): { subject: string; html: string } {
  ...
  const otpBlock = `
    ...
    <p style="color:VAR_ACCENT;font-size:72px;font-weight:900;letter-spacing:12px;text-align:center;margin:8px 0 4px;">${otp}</p>
    <p style="color:#5a8a6a;font-size:13px;text-align:center;margin-bottom:0;">Expires in ${WINDOW_MINUTES} minutes · Don't share this</p>
    <div style="border-top:1px solid #1a3825;margin:32px 0;"></div>
  `;

  const wrap = (accent: string, inner: string) =>
    `<div style="background:#050e08;color:#e8f5ee;font-family:Inter,Arial,sans-serif;padding:40px;max-width:480px;margin:0 auto;">${otpBlock.replace("VAR_ACCENT", accent)}${inner}${footer}</div>`;
...
export default async function handler(req: any, res: any) {
  ...
  if (!brevoKey || !otpSecret) return res.status(500).json({ error: "Server not configured" });
  ...
  sendEmail.replyTo = REPLY_TO;
  sendEmail.headers = EMAIL_HEADERS;
  ...
  try {
    ...
    console.error("[send-otp] Brevo error:", err?.message || err);
